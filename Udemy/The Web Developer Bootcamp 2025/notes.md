...
- **Inline elements** fit in alongside other elements:
  - **`<span>`**: A blank wrapper used to style a specific word or phrase inside a sentence.
- **Block level elements** take up a whole "block" of space:
  - **`<div>`(Content Division Element)**: Generic container used to group items together for styling or layout layout.

### An Odd Assortment of Elements

- **`<hr>`(The Horizontal Rule Element)**:
- **`<br>`(The Line Break Element)**: Makes a line break in text.
- **`<sup>`(The Supscript Element)**: Creates text that is rendered above the baseline.
- **`<sub>`(The Subscript Element)**: Creates text that is rendered below the baseline.
...
- **HTML Entities**: Special code/sequences that we can use inside of our HTML that will result in different characters, that normally would be invalid.
